...
and for after ,we have $Unemployment = 2.7331-1.5126*V+1.1667*1+-0.8679*V*1$ become $Unemployment = 3.889-2.3805*V$
...
#f)
This question depends on the research question, model in c is great if the assumption hold and the researcher question is about elastics. The problem was, if the research question is about elasticity of demand or supply, with demand and supply equation, this is a classic Simultaneous equations bias problem which all 3 result can be bias in which case non of them will be useful.
```{r}
model_full <- lm(Y ~ X1 + X2 + X3+X4+I(X1^2)+I(X2^2)+I(X3^2)+I(X4^2), data=df3)
summary(model_full) # show results
model_reduced <- lm(Y ~ X1 + X2 + X3+X4, data=df3)
anova(model_reduced, model_full)
```
The above regression have additional square term, in Taylor series approximation , an significant square term may indicate possible non linear relationship, but the anova F test suggest square term are jointly in signifcant
```{r}
